...
- [ ] `/reading` 可依程度與頁碼顯示正確 mock data，並還原有效 query。
- [ ] 每頁 20 篇；桌面一列 5 張，手機使用同一元件的橫向 compact 卡片。
- [ ] 無封面資料時顯示黑底白字預設封面。
- [ ] 換頁後捲動到第一張卡片，快速切換不產生過期捲動。
- [ ] `/reading/[id]` 使用固定模擬 nanoid 取得文章，無效 ID 顯示 404。
- [ ] 中日對照、僅顯示日文、單句學習模式互斥且畫面結果正確。
- [ ] 單句學習正確配對逐句日文與繁中翻譯，並顯示播放及收藏 UI。
- [ ] Ruby 開關與播放速度 UI 可操作；文章與單句可使用既有 TTS 播放，播放速度尚未套用。
- [ ] 文章使用結構化 paragraphs、sentences、segments，不使用 `v-html`。
- [ ] 點擊單字可使用 FlyonUI 官方 Popover 查看上下文解釋與例句。
- [ ] 點擊文法片段可定位並展開對應的文法 Collapse。
- [ ] 文法 Collapse 顯示短解釋、本文例句、額外例句與完整文法入口狀態。
- [ ] 顯示最多 5 篇同程度推薦文章並可正常導航。
- [ ] Header 電腦版及手機版均可前往 `/reading`。
- [ ] 桌面與手機版無非預期水平溢出。
