...
| 011 | 受控語音、教材資料庫與教材 API（v1 主線） | ✅ Completed |

## 現在焦點

- Branch：`dev`。
- 上一個完成：**Task 011**。三種教材已進 MongoDB，
...
- **階段 0** 的受控語音維持不變：`/api/tts` 只吃「教材類型＋教材 id＋單位 id」的座標，
  文字與 voice 都由伺服器決定。
- **閱讀／對話人工瀏覽器走查已完成**；單字 Popover 會標示單字教材收錄狀態與例句來源。
- **Task 009 再決定 rate limit**：若公開端只讀取預產音檔，付費合成入口維持私有；
  若保留公開即時合成 fallback，才需要限制合成請求頻率。
