...
✅ Completed（2026-08-16）。階段 0、階段 1 與收尾驗收完成；
eslint、build、API 驗證與人工瀏覽器走查皆通過。
...
- [x] 三個列表頁與兩個詳情頁的 Network 面板看得到 API 請求，畫面與改版前一致。
      （已用 SSR 輸出確認三個列表頁都渲染出卡片，並已完成人工瀏覽器走查。）
- [x] 切換程度與翻頁時有新的 API 請求（確認分頁真的在伺服器端）。
      （`?level=n5` 與 `?page=2` 直接打 API 都回不同結果；`page=99` 回空陣列且
      `totalPages` 仍為 5，前端靠它收斂顯示頁碼。）
- [x] 無效的教材 ID 顯示 404。
- [x] seed 連跑兩次，資料庫筆數不變。
- [x] 閱讀／對話頁同時存在「配對成功」與「未配對」的單字 popover，兩者都正常顯示。
      （使用者已完成人工走查；配對狀態與單字教材例句來源另於收尾加入畫面標示。）
...
- [x] **3-1** `npx eslint <changed-files>` 全數通過。
- [x] **3-2** `npm run build` 通過。
- [x] **3-3** 逐頁走一遍「Acceptance Criteria」的代表性情境。
- [x] **3-4** 更新 `context/current-feature.md`。
...
- **目前沒有 rate limit。** `TTS_ENABLED` 預設關閉，因此 Task 011 不把它當公開入口。
  Task 009 若改成預先產生並保存音檔，公開使用者只讀既有音檔，付費合成入口可維持私有；
  若仍保留公開即時合成 fallback，再於該 Task 加入 rate limit。
...
- [x] 代表性案例可正確顯示：有封面與 `null` 封面、配對成功與未配對的單字註解、
      有 Ruby 與無 Ruby 的句子、有文法註解與無文法註解的句子。
      （使用者已完成閱讀／對話人工瀏覽器走查。）
- [x] seed 可重跑，重跑後不產生重複教材，且回報 upsert 筆數。
      （9／9／86 的資料連跑兩次，報告與資料庫筆數完全相同。）
- [x] 單字自然鍵重複會被偵測並明確回報。
      （暫時加入第二筆 `食べる｜たべる｜verb` 後呼叫 seed，回傳 HTTP 409、兩筆 ID
      與「未寫入任何資料」；防護發生在資料庫連線與 bulk write 之前，測試資料已移除。）
...
- [x] 語音停用或失敗時，教材頁仍可閱讀，並有明確的失敗提示。
      （使用者已完成 `TTS_ENABLED` 關閉時的 503 Toast 人工驗收。）
